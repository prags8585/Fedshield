...
// (or a WebSocket) with Tab 2's — each tab gets its own five shells, wired to
// their own /ws/agents/tab3-* route in the backend (see agent_flow.py).
const AGENTS_WS_BASE = '/ws/agents'

const BRANCH_CHANNELS = [
  { key: 'tab3-branch-loc1', title: 'Branch — loc1' },
  { key: 'tab3-branch-loc2', title: 'Branch — loc2' },
  { key: 'tab3-branch-loc3', title: 'Branch — loc3' },
]
...
// Section 1 — Structuring Agent. Same shared structuring_log Redis list as
// Tab 2 (Agent 1's output isn't scenario-scoped), just polled from the
// /api/tab3/* prefix. Expect far more entries here (~451 flagged accounts
// vs ~27 for the 216 case).
function StructuringSection() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res = await fetch('/api/tab3/structuring-log')
...
// Section 2 — Money-Trail Agent. Grouped by ring_id server-side (19
// independent rings expected here, not just 1) so this shows one card per
// real ring rather than repeating the same ring's evidence per member token.
function MoneyTrailSection() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res = await fetch('/api/tab3/rings')
        const data = await res.json()
        if (!cancelled) {
          setRings(data.rings || [])
...
// Section 3 — Reporting Agent. Same shared reports/fraud_rings_report.xlsx
// as Tab 2 -- fires independent of the Verdict Agent's own opinion the
// moment ANY ring first converges (see CLAUDE.md).
function ReportSection() {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const res = await fetch('/api/tab3/report')
        const data = await res.json()
...
export default function Tab3() {
  ...
  const handleReset = async () => {
    ...
    setResetting(true)
    try {
      const res = await fetch('/api/tab3/reset', { method: 'POST' })
      if (!res.ok) {
  ...
  const handleStart = async () => {
    ...
    setStarting(true)
    try {
      const res = await fetch('/api/tab3/start', { method: 'POST' })
      if (!res.ok) {
  ...
  return (
    <div className="tab1">
      ...
      {/* <p className="viz-hint">
        19 independent laundering rings, ~451 flagged accounts (246 real fraud + ~205 false
        positives) — expect a long run (hours, not minutes). The Verdict Agent occasionally
        clears a structurally-real convergence as NOT_GUILTY (a known LLM-judgment gap, not a
        tracing bug). Avoid running scripts/test_agent_convergence.py or
        scripts/test_multi_ring_convergence.py while this is live — both wipe shared Redis/Neo4j
        state and will silently stall any investigation still in progress.
      </p> */}
